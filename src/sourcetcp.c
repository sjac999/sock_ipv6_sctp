...
/*
 * Invoked for a TCP client when sourcesink is set by the -i option.
 */
void
source_tcp(int sockfd)
{
	...
	for (i = 1; i <= nbuf; i++) {
		/*
		 * urgwrite is set to "n" by the "-U n" option.
		 * If and when "n" == i, the URG flag will be set in the
		 * TCP packet header, and the urgent pointer will be set
		 * to "n" in the TCP packet header.  Note that the data
